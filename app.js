...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    if(pd){
        ...
        // ОБНОВЛЕННАЯ ФУНКЦИЯ RENDER PROD (SEO + BREADCRUMBS + COPY)
        function renderProd(p){
            ...
            // BREADCRUMBS
            const bread = `<div class="breadcrumbs"><a href="index.html">Главная</a> <span>/</span> <a href="index.html">Каталог</a> <span>/</span> <span class="current">${p.sku}</span></div>`;

            // RENDER
            pd.innerHTML=`<div class="product-full-card" data-product-id="${p.id}">${bread}<div style="display:flex; gap:50px; flex-wrap:wrap; width:100%;"><div class="gallery-container" style="flex:1;min-width:300px;"><div class="full-img-wrapper" id="mainImgWrapper" onclick="openLightbox(${aj},0)"><img id="productMainImg" src="${m}" alt="${p.name}" onerror="this.src='${SITE_CONFIG.placeholderImage}'"></div>${th}</div><div class="full-info"><div class="full-sku copy-sku" onclick="copyToClipboard('${p.sku}')" title="Копировать" style="display:inline-block; width:auto;">АРТИКУЛ: ${p.sku}</div><h1 class="full-title">${p.name}</h1><div class="full-price">${pr}</div><div class="full-actions-group"><a href="index.html" class="btn-detail blue">В КАТАЛОГ</a><button id="btn-add-${p.id}" onclick="addToCart('${p.id}','${p.sku}','${n}','${pr}')" class="btn-detail green">В КОРЗИНУ</button><button class="btn-fav-full ${isF}" onclick="toggleFav('${p.id}',event)" data-fav-id="${p.id}"><svg viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg></button><div id="btn-qty-${p.id}" class="btn-qty-grid hidden"><button onclick="updateItemQty('${p.id}',-1)">-</button><span id="qty-val-${p.id}">1</span><button onclick="updateItemQty('${p.id}',1)">+</button></div></div><div class="full-desc"><strong>Описание:</strong><br>${p.desc||'Нет описания'}<br><br><strong>Категория:</strong> ${p.category||'-'}</div></div></div></div>`;
            window.updateCartUI();updateFavUI();
        }
